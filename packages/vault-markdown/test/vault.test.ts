...
import type { Blob, Record } from '../src/model.ts';

import { MarkdownVault } from '../src/index.ts';
import {
  InvalidContentError,
...
`,
  ...
  it('rejects two collections whose names differ only by case', async () => {
    // Their folders would be one.
    const root = await vault({
      'autofile.yml': 'collections:\n  contacts: { type: record }\n  Contacts: { type: record }\n',
    });
...
`,
  ...
  it('writes an object carrying more than a $ref as the mapping it is', async () => {
    // A reference is an object whose only key is `$ref`. One carrying anything
    // alongside it is ordinary data, and a wikilink would drop what it holds.
    const root = await vault({ 'autofile.yml': COLLECTIONS });
    const opened = await MarkdownVault.open(root);

    await opened.put('contacts/priya-narayan', {
      photo: { $ref: 'blobs/contacts/priya-narayan.jpg', label: 'at the press' },
    });
    ...
    assert.equal(
      ...
      `---
photo:
  $ref: blobs/contacts/priya-narayan.jpg
  label: at the press
---
`,
    );
    assert.deepEqual(record(await opened.get('contacts/priya-narayan')).fields, {
      photo: { $ref: 'blobs/contacts/priya-narayan.jpg', label: 'at the press' },
    });
  });
...
describe('MarkdownVault.remove', () => {
  ...
  it('is an error to remove what is not there', async () => {
    // Rather than answering differently from `get` about the same absence.
    const root = await vault({ 'autofile.yml': COLLECTIONS });
    const opened = await MarkdownVault.open(root);

    await assert.rejects(opened.remove('contacts/nobody'), NotFoundError);
    await assert.rejects(opened.remove('blobs/nothing/here.jpg'), NotFoundError);
    await assert.rejects(opened.remove('blobs/contacts'), NotFoundError);
  });
  ...
  it('leaves a record where a blob key names its file', async () => {
    ...
    // The blob collection does not hold a record's file, so there is nothing at
    // this identity to remove — which is what `get` answers about it too.
    await assert.rejects(opened.remove('blobs/contacts/priya-narayan.md'), NotFoundError);
...
`,
  ...
  it('reports two identities whose files differ only by case', async () => {
    const root = await vault({
      'autofile.yml': COLLECTIONS,
      'contacts/Priya-Narayan.md': '',
      'contacts/priya-narayan.md': '',
    ...
    assert.deepEqual(await violations(root), [
      {
        ...
        id: 'contacts/Priya-Narayan',
        collection: 'contacts',
        message: "its file differs only by case from 'contacts/priya-narayan'",
      },
      {
        rule: 'collision',
        severity: 'violation',
  ...
  it("reports a record's file colliding with a blob's", async () => {
    // Checked over the whole vault rather than within a collection: records and
    // blobs share one tree, so a record's file can collide with a blob's.
    const root = await vault({
      'autofile.yml': COLLECTIONS,
      'contacts/priya-narayan.md': '',
      'Contacts/priya-narayan.md': '',
    ...
    assert.deepEqual(await violations(root), [
      {
        rule: 'collision',
        severity: 'violation',
        id: 'blobs/Contacts/priya-narayan.md',
        collection: 'blobs',
        message: "its file differs only by case from 'contacts/priya-narayan'",
      },
      {
        ...
        id: 'contacts/priya-narayan',
        collection: 'contacts',
        message: "its file differs only by case from 'blobs/Contacts/priya-narayan.md'",
      },
    ]);
  });

  it('reports no collision between identities whose files differ', async () => {
    // What is folded is the file each identity claims rather than its key: a
    // record's file gains the `.md` a blob's key spells out in full, so these
    // two are two files however they are spelled.
    const root = await vault({
      'autofile.yml': COLLECTIONS,
      'contacts/priya-narayan.md': '',
      'contacts/priya-narayan': '',
